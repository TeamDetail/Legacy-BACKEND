...
public class StoreController {
    ...
    public ResponseEntity<BaseResponse<CardPackRes>> getStorePage(@PathVariable Long userId) {
//        Long userId = userSessionHolder.get().getUserId();
        return BaseResponse.of(storeService.getCardPack(userId));
    }

    @PatchMapping("/cardBuy/{cardpackId}/{userId}")
    @Operation(summary = "카드팩 구매", description = "카드팩을 하나 구매합니다.")
    public ResponseEntity<BaseResponse<String>> buyCardPack(@PathVariable Long cardpackId, @PathVariable Long userId) {
//        Long userId = userSessionHolder.get().getUserId();
